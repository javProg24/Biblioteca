...
public class PropiedadesCreator {
    ...
    private static int index=1; // Índice para los parámetros, comienza en 1 para evitar el ID
    ...
    public <T extends Entidad> Map<Integer, Object> listarPropiedad(T entidad,String atributo){// para eliminar y consultas de 1 parametro
        lista.clear();
        try {
            Class<?> clazz = entidad.getClass(); // obtiene la clase de la entidad
            String nombreGetter = metodo  + atributo; // construye el nombre del metodo getter correspondiente al atributo
            Method getter = clazz.getMethod(nombreGetter); // obtiene el metodo getter de la clase
